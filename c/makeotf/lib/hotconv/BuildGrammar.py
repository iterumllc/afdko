...
lex_out = ['FeatLexer.cpp', 'FeatLexer.h', 'FeatLexer.interp', 'FeatLexer.tokens']

parse_in = ['FeatParser.g4']
parse_out = ['FeatParser.cpp', 'FeatParser.h', 'FeatParserVisitor.h',
             'FeatParserVisitor.cpp', 'FeatParser.interp', 'FeatParserBaseVisitor.h',
             'FeatParserBaseVisitor.cpp', 'FeatParser.tokens']

parser = argparse.ArgumentParser()
parser.add_argument('-c', '--clean', help="remove generated files", action="store_true")
parser.add_argument('-n', '--dry-run', help="just print what the program would do", action="store_true")
parser.add_argument('-f', '--force', help="Regenerate even if current files are newer", action="store_true")
...
def run_program(prog):
    print(' '.join(prog))
    if not args.dry_run:
        a = subprocess.run(prog, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
...
if ( args.clean ):
    ...
    if not parser_ok or args.force:
        run_program([antlr_program] + antlr_args + ['-visitor'] + [parse_in[0]])
